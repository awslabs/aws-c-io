...
`BUILD_RELOCATABLE_BINARIES` | Linux | Builds a binary that runs on target kernels older than the build host, for prebuild pipelines that compile on an old toolchain (defines `COMPAT_MODE`). The Linux event loop then avoids newer facilities those old kernels lack: cross-thread notification falls back from `eventfd` to a pipe, and scheduled-task wakeups fall back from a `CLOCK_BOOTTIME` timerfd (which keeps tasks on time across device suspend) to the `CLOCK_MONOTONIC` `epoll_wait` timeout. Enable only when the build host's kernel is newer than the oldest kernel you must support. | OFF
