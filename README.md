...
Windows | Overrides the system's trust, and only allows TLS handshakes if the peer is verifiable from the user supplied CA
Linux/BSD | Appends user supplied CA to the same CA/trust as the linked libcrypto (i.e. /etc/ssl/certs). TLS handshakes will succeed if the peer can be verified from either the user supplied CA or the linked libcrypto trust.
