...
struct socket_test_args {
    ...
    bool destruction_callback_invoked;
...
static void s_socket_channel_destruction_callback(void *context) {
    struct socket_test_args *test_args = context;
...
static bool s_socket_channel_destruction_callback_invoked_pred(void *user_data) {
    struct socket_test_args *test_args = user_data;
    return test_args->destruction_callback_invoked;
}

static int s_socket_bootstrap_destruction_callback_fn(struct aws_allocator *allocator, void *ctx) {
    (void)ctx;

    uint64_t default_wait_time = aws_timestamp_convert(2, AWS_TIMESTAMP_SECS, AWS_TIMESTAMP_NANOS, NULL);
    ...
    client_channel_options.setup_callback = s_socket_handler_test_client_setup_callback;
    client_channel_options.shutdown_callback = s_socket_handler_test_client_shutdown_callback;
    client_channel_options.channel_destruction_callback = s_socket_channel_destruction_callback;
    client_channel_options.enable_read_back_pressure = false;
    client_channel_options.user_data = &client_args;
    ...
    ASSERT_SUCCESS(aws_condition_variable_wait_for_pred(
        ...
        &c_tester.mutex,
        default_wait_time,
        s_pinned_channel_setup_predicate,
        &server_args));
    ASSERT_SUCCESS(aws_condition_variable_wait_for_pred(
        &c_tester.condition_variable,
        &c_tester.mutex,
    ...
    ASSERT_SUCCESS(aws_condition_variable_wait_for_pred(
        &c_tester.condition_variable, &c_tester.mutex, default_wait_time, s_channel_shutdown_predicate, &server_args));
    ASSERT_SUCCESS(aws_condition_variable_wait_for_pred(
        &c_tester.condition_variable, &c_tester.mutex, default_wait_time, s_channel_shutdown_predicate, &client_args));
    aws_server_bootstrap_destroy_socket_listener(local_server_tester.server_bootstrap, local_server_tester.listener);
    ASSERT_SUCCESS(aws_condition_variable_wait_for_pred(
        &c_tester.condition_variable, &c_tester.mutex, default_wait_time, s_listener_destroy_predicate, &server_args));

    ASSERT_SUCCESS(aws_condition_variable_wait_for_pred(
        &c_tester.condition_variable,
        &c_tester.mutex,
        default_wait_time,
        s_socket_channel_destruction_callback_invoked_pred,
        &client_args));
    ASSERT_TRUE(client_args.destruction_callback_invoked);
    aws_mutex_unlock(&c_tester.mutex);
...
AWS_TEST_CASE(socket_bootstrap_destruction_callback, s_socket_bootstrap_destruction_callback_fn)