...
static int s_socket_handler_test_client_setup_callback(struct aws_client_bootstrap *bootstrap, int error_code,
...
static int s_socket_handler_test_server_setup_callback(struct aws_server_bootstrap *bootstrap, int error_code,
                                                       struct aws_channel *channel, void *user_data) {
    struct socket_test_args *setup_test_args = (struct socket_test_args *)user_data;
...
static int s_socket_handler_test_client_shutdown_callback(struct aws_client_bootstrap *bootstrap, int error_code,
                                                          struct aws_channel *channel, void *user_data) {
    struct socket_test_args *setup_test_args = (struct socket_test_args *)user_data;
...
static int s_socket_handler_test_server_shutdown_callback(struct aws_server_bootstrap *bootstrap, int error_code,
                                                          struct aws_channel *channel, void *user_data) {
    struct socket_test_args *setup_test_args = (struct socket_test_args *)user_data;
...
static struct aws_byte_buf s_socket_test_handle_read(struct aws_channel_handler *handler, struct aws_channel_slot *slot,
...
static struct aws_byte_buf s_socket_test_handle_write(struct aws_channel_handler *handler, struct aws_channel_slot *slot,
                                               struct aws_byte_buf *data_read, void *user_data) {
    /*do nothing*/
...
static int s_socket_echo_and_backpressure_test(struct aws_allocator *allocator, void *user_data) {
    ...
    struct aws_channel_handler *outgoing_rw_handler = s_rw_test_handler_new(allocator, s_socket_test_handle_read,
                                                                            s_socket_test_handle_write, true,
                                                                            read_tag.len / 2, &outgoing_rw_args);
    ASSERT_NOT_NULL(outgoing_rw_handler);

    struct aws_channel_handler *incoming_rw_handler = s_rw_test_handler_new(allocator, s_socket_test_handle_read,
                                                                            s_socket_test_handle_write, true,
                                                                            write_tag.len / 2, &incoming_rw_args);
    ASSERT_NOT_NULL(outgoing_rw_handler);
    ...
    ASSERT_SUCCESS(aws_condition_variable_wait_pred(&condition_variable, &mutex, s_socket_test_read_predicate,
                                                    &incoming_rw_args));
...
static int s_socket_close_test(struct aws_allocator *allocator, void *user_data) {
    ...
    struct aws_channel_handler *outgoing_rw_handler = s_rw_test_handler_new(allocator, s_socket_test_handle_read,
                                                                            s_socket_test_handle_write, true, 10000,
                                                                            &outgoing_rw_args);
    ASSERT_NOT_NULL(outgoing_rw_handler);
    ...
    ASSERT_SUCCESS(s_rw_handler_wait_on_shutdown(outgoing_args.rw_handler));
    ASSERT_INT_EQUALS(AWS_IO_SOCKET_CLOSED, s_rw_handler_last_error_code(outgoing_args.rw_handler));
