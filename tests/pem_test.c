...
static int s_test_der_cert_to_pem_typical(struct aws_allocator *allocator, void *ctx) {
    (void)ctx;

    static const uint8_t s_der_cert[] = {
        0x30, 0x82, 0x02, 0x78, 0x30, 0x82, 0x01, 0xe1, 0xa0, 0x03, 0x02, 0x01, 0x02, 0x02, 0x09, 0x00, 0xe6, 0xed,
        0xb6, 0x73, 0xca, 0x42, 0x15, 0x65, 0x30, 0x0d, 0x06, 0x09, 0x2a, 0x86, 0x48, 0x86, 0xf7, 0x0d, 0x01, 0x01,
        0x0e, 0x05, 0x00, 0x30, 0x5f, 0x31, 0x0b, 0x30, 0x09, 0x06, 0x03, 0x55, 0x04, 0x06, 0x13, 0x02, 0x55, 0x53,
        0x31, 0x0b, 0x30, 0x09, 0x06, 0x03, 0x55, 0x04, 0x08, 0x0c, 0x02, 0x57, 0x41, 0x31, 0x10, 0x30, 0x0e, 0x06,
    ...
    struct aws_pem_object *pem_object = NULL;
    aws_array_list_get_at_ptr(&pem_objects, (void **)&pem_object, 0);
    ASSERT_BIN_ARRAYS_EQUALS(s_der_cert, sizeof(s_der_cert), pem_object->data.buffer, pem_object->data.len);
    ASSERT_INT_EQUALS(AWS_PEM_TYPE_X509, pem_object->type);
...
static int s_test_der_cert_to_pem_small(struct aws_allocator *allocator, void *ctx) {
    (void)ctx;

    /* 10 bytes - smaller than AWS_DER_MAX_CHUNK_SIZE (48), produces single base64 line */
...
static int s_test_der_cert_to_pem_output_format(struct aws_allocator *allocator, void *ctx) {
    (void)ctx;

    /* 96 bytes - exactly 2 * AWS_DER_MAX_CHUNK_SIZE, produces exactly 2 full base64 lines */
    uint8_t s_der[96];
    ...
    aws_byte_cursor_advance(&body, expected_header.len);
    /* With 96 bytes input, we get exactly 2 lines of 64 base64 chars each */
    struct aws_byte_cursor substr;
    AWS_ZERO_STRUCT(substr);
    for (int i = 0; i < 2; i++) {
        ASSERT_TRUE(aws_byte_cursor_next_split(&body, '\n', &substr));
...
static int s_test_der_cert_to_pem_empty_input_fails(struct aws_allocator *allocator, void *ctx) {
    (void)ctx;
...
AWS_TEST_CASE(test_der_cert_to_pem_empty_input_fails, s_test_der_cert_to_pem_empty_input_fails)
