...
static int s_test_uri_full_parse(struct aws_allocator *allocator, void *ctx) {
    ...
    ASSERT_BIN_ARRAYS_EQUALS(
        expected_request_uri.ptr, expected_request_uri.len, uri.request_uri.ptr, uri.request_uri.len);
...
static int s_test_request_uri_only_parse(struct aws_allocator *allocator, void *ctx) {
    (void)ctx;
...
AWS_TEST_CASE(request_uri_only_parse, s_test_request_uri_only_parse);
...
static int s_test_uri_query_params(struct aws_allocator *allocator, void *ctx) {
    ...
    struct aws_uri_params params[4] = {0};
    struct aws_array_list params_list;
    aws_array_list_init_static(&params_list, &params, 4, sizeof(struct aws_uri_params));

    ASSERT_SUCCESS(aws_uri_query_string_params(&uri, &params_list));
    ASSERT_UINT_EQUALS(4u, aws_array_list_length(&params_list));
    ...
    ASSERT_BIN_ARRAYS_EQUALS(expected_value.ptr, expected_value.len, params[1].value.ptr, params[1].value.len);
...
static int s_test_invalid_scheme_parse(struct aws_allocator *allocator, void *ctx) {
    (void)ctx;
    const char *str_uri =
        "https:/www.test.com:8443/path/to/resource?test1=value1&test%20space=value%20space&test2=value2&test2=value3";
...
AWS_TEST_CASE(invalid_scheme_parse, s_test_invalid_scheme_parse);

static int s_test_invalid_port_parse(struct aws_allocator *allocator, void *ctx) {
    (void)ctx;
    const char *str_uri =
...
AWS_TEST_CASE(invalid_port_parse, s_test_invalid_port_parse);

static int s_test_port_too_large_parse(struct aws_allocator *allocator, void *ctx) {
    (void)ctx;
...
AWS_TEST_CASE(port_too_large_parse, s_test_port_too_large_parse);
...
static int s_test_uri_builder(struct aws_allocator *allocator, void *ctx) {
    ...
    struct aws_uri_builder_args builder_args = {
        .scheme = uri.scheme,
        .path = uri.path,
    ...
    ASSERT_SUCCESS(aws_uri_init(&built_uri, allocator, &builder_args));
    ...
    struct aws_byte_cursor expected_request_uri = aws_byte_cursor_from_c_str(
        "/path/to/resource?test1=value1&test%20space=value%20space&test2=value2&test2=value3");
    ASSERT_BIN_ARRAYS_EQUALS(
        expected_request_uri.ptr, expected_request_uri.len, built_uri.request_uri.ptr, built_uri.request_uri.len);
