...
static int s_test_read_write_notifications(struct aws_allocator *allocator, void *ctx) {
    ...
    size_t written = simple_pipe_write(&write_data.handle, write_buffer, 512);
    ASSERT_UINT_EQUALS(512, written);

    ASSERT_SUCCESS(s_wait_for_next_invocation(&read_data));

    written = simple_pipe_write(&write_data.handle, write_buffer + 512, 512);
    ASSERT_UINT_EQUALS(512, written);
    ...
    simple_pipe_close(&read_data.handle, &write_data.handle);
