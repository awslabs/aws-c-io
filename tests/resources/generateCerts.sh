...
# Certificates in this folder that are intentionally NOT regenerated by this script
# (do NOT add them here): testparse.crt, testparse_crlf.crt, testcert0-7.pem,
# ecc-cert.pem/ecc-key.pem, and DigiCertGlobalRootCA.crt.pem. These are static test
# fixtures. The testparse*/testcert*/ecc-* certs are only used by PEM-parsing and certificate-import
# tests that assert against hardcoded byte content, so regenerating them would break those tests.
# DigiCertGlobalRootCA is a real public root CA used as a negative test case and is not ours to regenerate.
...
for base in unittests ec_unittests; do
  ...
  # Use SHA1/3DES-based PBE instead of OpenSSL 3.x's modern AES-256/PBES2 defaults.
  # macOS's Security Framework (SecPKCS12Import) cannot parse the modern defaults and
  # fails with "MAC verification failed", and unittests.p12 is loaded by the Apple
  # SecureTransport tests. See tests/resources/README.md.
