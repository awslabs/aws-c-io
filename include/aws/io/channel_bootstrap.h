...
/**
 * If TLS is being used, this function is called once the socket has connected, the channel has been initialized, and TLS
 * has been successfully negotiated. A TLS handler has already been added to the channel. If TLS negotiation fails, this function
 * will be called with the corresponding error code.
 *
 ...
 *
 * If an error occurred, you do not need to shutdown the channel. The `aws_channel_client_shutdown_callback` will be invoked
...
/**
 * If ALPN is being used, this function will be invoked by the channel once an ALPN message is received. The returned channel_handler will
 * be added to, and managed by, the channel.
 */
...
/**
 * If TLS is being used, this function is called once the socket has received an incoming connection, the channel has been initialized, and TLS
 * has been successfully negotiated. A TLS handler has already been added to the channel. If TLS negotiation fails, this function
 * will be called with the corresponding error code.
 *
...
/**
 * Initializes the client bootstrap with `allocator` and `el_group`. This object manages client connections and channels.
 */
...
/**
 * Cleans up the bootstrap's resources. Does not clean up any of your channels. You must shutdown your channels before calling this if you
 * don't want a memory leak.
 */
AWS_IO_API void aws_client_bootstrap_clean_up (struct aws_client_bootstrap *bootstrap);

/**
 * Sets the tls context for use with `aws_client_bootstrap_new_tls_socket_channel`. This function must be called before calling,
 * `aws_client_bootstrap_new_tls_socket_channel`
 */
...
/**
 * Sets up a client socket channel. If you are planning on using tls, use `aws_client_bootstrap_new_tls_socket_channel` instead.
 * The connection is made to `endpoint` using socket options `options`. `setup_callback` will be invoked once the channel is ready for use
 * or if an error is encountered. `shutdown_callback` will be invoked once the channel has shutdown. Immediately after the `shutdown_callback`
 * returns, the channel is cleaned up automatically. All callbacks are invoked in an event-loop's thread.
 */
...
/**
 * Sets up a client tls socket channel. The connection is made to `endpoint` using socket options `options`, and tls options `connection_options`.
 * `setup_callback` will be invoked once the channel is ready for use and TLS has been negotiated,
 * or if an error is encountered. `shutdown_callback` will be invoked once the channel has shutdown. Immediately after the `shutdown_callback`
 * returns, the channel is cleaned up automatically. All callbacks are invoked in an event-loop's thread.
 *
...
/**
 * Sets the tls context for use with `aws_server_bootstrap_add_tls_socket_listener`. This function must be called before calling,
 * `aws_server_bootstrap_add_tls_socket_listener`
 */
...
/**
 * Sets up a server socket listener. If you are planning on using tls, use `aws_server_bootstrap_add_tls_socket_listener` instead.
 * This creates a socket listener bound to `endpoint` using socket options `options`. `incoming_callback` will be invoked
 * once an incoming channel is ready for use or if an error is encountered. `shutdown_callback` will be invoked once the channel
 * has shutdown. Immediately after the `shutdown_callback` returns, the channel is cleaned up automatically.
 * All callbacks are invoked in an event-loop's thread.
 *
 * Upon shutdown of your application, you'll want to call `aws_server_bootstrap_remove_socket_listener` with the return value from this function.
 */
 ...
 * This creates a socket listener bound to `endpoint` using socket options `options`, and tls options `connection_options`.
 * `incoming_callback` will be invoked once an incoming channel is ready for use and TLS is finished negotiating,
 * or if an error is encountered. `shutdown_callback` will be invoked once the channel has shutdown.
 * Immediately after the `shutdown_callback` returns, the channel is cleaned up automatically. All callbacks are
 * invoked in an event-loop's thread.
 *
 * Upon shutdown of your application, you'll want to call `aws_server_bootstrap_remove_socket_listener` with the return value from this function.
 */
...
/**
 * Shuts down 'listener' and cleans up any resources associated with it. Any incoming channels on `listener` will still be active.
 */
