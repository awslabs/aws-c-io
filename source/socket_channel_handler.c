...
static void s_do_read(struct socket_handler *socket_handler) {
    ...
    /* Guard against the race condition where a readable event fires after the setup callback
    ...
    while (total_read < max_to_read) {
        ...
        if (socket_handler->slot->adj_right == NULL ||
            aws_channel_slot_send_message(socket_handler->slot, message, AWS_CHANNEL_DIR_READ)) {
            last_error = aws_last_error();
            aws_mem_release(message->allocator, message);
            break;
        }
