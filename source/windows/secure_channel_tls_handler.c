...
static int s_process_read_message(
    ...
    while (!err && message_cursor.len) {
        ...
        if (err) {
            /* AWS_IO_READ_WOULD_BLOCK isn't really an error, it just means the record is incomplete */
        ...
        if (record_is_incomplete) {
            /* if the buffer is full, but it's an incomplete record ... throw this one as a protocol error. */
