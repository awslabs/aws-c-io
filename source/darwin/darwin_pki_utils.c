...
int aws_import_public_and_private_keys_to_identity(
    ...
    if (keychain_path) {
        ...
        if (keychain_status != errSecSuccess) {
            ...
            aws_log_security_osstatus(AWS_LL_ERROR, AWS_LS_IO_PKI, "SecKeychainOpen()", keychain_status);
            AWS_LOGF_ERROR(
                AWS_LS_IO_PKI,
                "static: error opening keychain \"%s\", error %d (%s)",
        ...
        if (keychain_status != errSecSuccess) {
            aws_raise_error(AWS_ERROR_SYS_CALL_FAILURE);
            // ******************************************************************************************************************************
            aws_log_security_osstatus(AWS_LL_ERROR, AWS_LS_IO_PKI, "SecKeychainUnlock()", keychain_status);
            AWS_LOGF_ERROR(
                AWS_LS_IO_PKI,
                "static: error unlocking keychain \"%s\", error %d (%s)",
                aws_string_c_str(keychain_path),
                aws_last_error(),
                aws_error_name(aws_last_error()));
            return AWS_OP_ERR;
        ...
        if (keychain_status != errSecSuccess) {
            aws_raise_error(AWS_ERROR_SYS_CALL_FAILURE);
            aws_log_security_osstatus(AWS_LL_ERROR, AWS_LS_IO_PKI, "SecKeychainCopyDefault()", keychain_status);
            AWS_LOGF_ERROR(
                AWS_LS_IO_PKI,
                "static: error opening the default keychain, error %d (%s)",
                aws_last_error(),
                aws_error_name(aws_last_error()));
            return AWS_OP_ERR;
        }
    ...
    if (cert_status != errSecSuccess && cert_status != errSecDuplicateItem) {
        aws_raise_error(AWS_ERROR_SYS_CALL_FAILURE);
        aws_log_security_osstatus(AWS_LL_ERROR, AWS_LS_IO_PKI, "SecItemImport()", cert_status);
        AWS_LOGF_ERROR(
            AWS_LS_IO_PKI,
            "static: error importing certificate, error %d (%s)",
            aws_last_error(),
            aws_error_name(aws_last_error()));
    ...
    if (key_status != errSecSuccess && key_status != errSecDuplicateItem) {
        aws_raise_error(AWS_ERROR_SYS_CALL_FAILURE);
        aws_log_security_osstatus(AWS_LL_ERROR, AWS_LS_IO_PKI, "SecItemImport()", key_status);
        AWS_LOGF_ERROR(
            AWS_LS_IO_PKI,
            "static: error importing private key, error %d (%s)",
            aws_last_error(),
            aws_error_name(aws_last_error()));
        result = aws_raise_error(AWS_IO_FILE_VALIDATION_FAILURE);
        goto done;
...
int aws_import_pkcs12_to_identity(
    ...
    aws_log_security_osstatus(AWS_LL_ERROR, AWS_LS_IO_PKI, "SecPKCS12Import()", status);
